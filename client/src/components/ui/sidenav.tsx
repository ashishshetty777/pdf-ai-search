...
import Logo from '@/assets/iotriclogo.png'
      ...
      >
        <div className={`flex mb-10 items-baseline ${isCollapsed ? 'flex-col gap-2' : 'flex-row'}`}>
          ...
          >
            ...
            <span className={`${isCollapsed ? 'hidden' : 'inline font-medium'}`}><Image src={Logo} width={150} alt="logo image" /></span>
          </div>
        </div>
        ...
        <ul className="!overflow-visible">
          ...
          <SidenavItem
            ...
            isCollapsed={isCollapsed}
            icon={<FaPlus className={`${isCollapsed ? '' : 'scale-75'} ${isActive('/workspace/new') ? 'text-white' : 'text-[#002e5d]'}`} />}
            animatedBorder
          />
        </ul>
      </div>
      ...
      {isCollapsed ? (
        <Tooltip text={<span>{title}{locked && <span className="ml-2">(Read-Only)</span>}</span>}>
          <Link
            ...
            ${isActive ? 'bg-[#002e5d] text-white transition-colors duration-0'
